...
export async function POST(request: Request) {
    try {
        ...
        const vip_until = new Date(Date.now() + hours * 60 * 60 * 1000).toISOString();
...
export async function PATCH(request: Request) {
    try {
        ...
        const now = new Date();
        const currentUntil = new Date(current.vip_until);
        const baseTime = currentUntil > now ? currentUntil : now;
        const new_vip_until = new Date(baseTime.getTime() + additional_hours * 60 * 60 * 1000).toISOString();

        const { data, error } = await supabase
            .from('vip_listings')
            .update({ vip_until: new_vip_until })
            .eq('listing_id', listing_id)
            .select()
            .single();
