...
import {
    ...
    Bell, Star, BarChart3, Menu, X, Users
} from 'lucide-react';
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-black flex">
            {/* Sidebar Desktop */}
            <aside className="hidden lg:flex w-72 bg-gray-50 dark:bg-[#0a0a0a] border-r border-gray-100 dark:border-white/5 flex-col sticky top-0 h-screen transition-all duration-500 hover:shadow-2xl hover:shadow-primary/5">
                <div className="p-6">
                    <Link href="/" className="flex items-center gap-2 group">
                        <div className="w-9 h-9 bg-gold-gradient rounded-xl flex items-center justify-center group-hover:rotate-12 transition-transform shadow-lg shadow-primary/20">
                            <LayoutDashboard className="text-black w-5 h-5" />
                        </div>
                        <span className="text-xl font-black uppercase tracking-tighter text-gray-900 dark:text-white">Panel</span>
                    </Link>
                </div>
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-center gap-3 relative z-10">
                                    ...
                                    <span className="font-bold text-xs tracking-tight uppercase">{item.name}</span>
                                </div>
                ...
                <div className="p-4 border-t border-gray-100 dark:border-white/5">
                    <div className="bg-white dark:bg-white/5 rounded-2xl p-4 flex items-center gap-3 group cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10 transition-all border border-gray-100 dark:border-white/5 shadow-sm dark:shadow-none">
                        <div className="w-10 h-10 rounded-xl bg-gray-50 dark:bg-black border border-gray-100 dark:border-white/10 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                            <div className="w-7 h-7 rounded-lg bg-primary/10 flex items-center justify-center text-primary font-black uppercase tracking-widest text-[10px]">
                                {user.displayName?.[0] || user.email?.[0] || 'U'}
                            </div>
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className="font-black text-gray-900 dark:text-white truncate tracking-tight text-xs uppercase">{user.displayName || 'Kullanıcı'}</p>
                            <p className="text-[9px] font-black text-primary uppercase tracking-widest opacity-60">
                                {user.isAdmin ? 'Yönetici' : (roleLabels[userType as keyof typeof roleLabels] || 'Üye')}
                            </p>
                        </div>
                        <button
                            onClick={handleLogout}
                            className="w-9 h-9 rounded-xl hover:bg-red-500/10 dark:hover:bg-red-500/20 hover:text-red-600 dark:hover:text-red-500 transition-colors flex items-center justify-center text-gray-400 dark:text-gray-500"
                        >
